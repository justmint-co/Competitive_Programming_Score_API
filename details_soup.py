...
class UserData:
    ...
    def __codechef(self):
        ...
        highest_rating = (
            highest_rating_container.find_next("small").text.split()[-1].rstrip(")")
        )
        ...
        def contests_details_get():
            ...
            try:
                long_challenge = {
                    "name": "Long Challenge",
                    "rating": int(rating_table_rows[1].text),
            ...
            try:
                lunch_time = {
                    "name": "Lunch Time",
                    "rating": int(rating_table_rows[9].text),
        ...
        def problems_solved_get():
            ...
            if fully_solved["count"] != 0:
                for category in categories[0].find_all("p"):
                    ...
                    for prob in category.find_all("a"):
                        fully_solved[category_name].append(
                            {
                                "name": prob.text,
            ...
            if partially_solved["count"] != 0:
                for category in categories[1].find_all("p"):
                    ...
                    for prob in category.find_all("a"):
                        partially_solved[category_name].append(
                            {
                                "name": prob.text,
                                "link": "https://www.codechef.com" + prob["href"],
                            }
    ...
    def __codeforces(self):
        urls = {
            ...
            "user_contests": {
                "url": f"https://codeforces.com/contests/with/{self.__username}"
            },
        }
    ...
    def __spoj(self):
        ...
        join_date = (
            ...
            + " "
            + details_container[1].text.split()[2]
        )
    ...
    def __leetcode(self):
        ...
        try:
            ...
            ranking = driver.find_element_by_xpath(
                "/html/body/div[4]/div/div/div/div[2]"
            ).text
            print("rank: ", ranking)

            total_problems_solved = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[2]/div/div[1]/div[1]/div[2]"
            ).text

            acceptance_rate_span_1 = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[2]/div/div[1]/div[2]/div[2]/div/div[1]/span[1]"
            ).text
            acceptance_rate_span_2 = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[2]/div/div[1]/div[2]/div[2]/div/div[1]/span[2]"
            ).text
            acceptance_rate = str(acceptance_rate_span_1) + str(acceptance_rate_span_2)

            easy_questions_solved = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[1]/div[2]/span[1]'
            ).text
            total_easy_questions = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[1]/div[2]/span[2]'
            ).text

            medium_questions_solved = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[2]/div[2]/span[1]'
            ).text
            total_medium_questions = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[2]/div[2]/span[2]'
            ).text

            hard_questions_solved = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[3]/div[2]/span[1]'
            ).text
            total_hard_questions = driver.find_element_by_xpath(
                '//*[@id="profile-root"]/div[2]/div/div[1]/div[2]/div/div[2]/div/div[3]/div[2]/span[2]'
            ).text

            contribution_points = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[3]/div[2]/div/div/div/li[1]/span"
            ).text

            contribution_problems = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[3]/div[2]/div/div/div/li[2]/span"
            ).text

            contribution_testcases = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[3]/div[2]/div/div/div/li[3]/span"
            ).text

            reputation = driver.find_element_by_xpath(
                "/html/body/div[1]/div[2]/div/div[1]/div[4]/div[2]/div/div/div/li/span"
            ).text
        finally:
            driver.close()
            driver.quit()
    ...
    def __leetcode_v2(self):
        def __parse_response(response):
            ...
            total_questions_stats = get_safe_nested_key(
            ...
            total_submissions = get_safe_nested_key(
                ["data", "matchedUser", "submitStats", "totalSubmissionNum"], response
            )
            ...
            if total_submissions_count > 0:
                acceptance_rate = round(
                    ac_submissions_count * 100 / total_submissions_count, 2
                )
            if total_easy_submissions_count > 0:
                easy_acceptance_rate = round(
                    ac_easy_submissions_count * 100 / total_easy_submissions_count, 2
                )
            if total_medium_submissions_count > 0:
                medium_acceptance_rate = round(
                    ac_medium_submissions_count * 100 / total_medium_submissions_count,
                    2,
                )
            if total_hard_submissions_count > 0:
                hard_acceptance_rate = round(
                    ac_hard_submissions_count * 100 / total_hard_submissions_count, 2
                )

            contribution_points = get_safe_nested_key(
                ["data", "matchedUser", "contributions", "points"], response
            )
            contribution_problems = get_safe_nested_key(
                ["data", "matchedUser", "contributions", "questionCount"], response
            )
            contribution_testcases = get_safe_nested_key(
                ["data", "matchedUser", "contributions", "testcaseCount"], response
            )
